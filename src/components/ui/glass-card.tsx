...
function GlassCard({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex flex-col gap-6 rounded-2xl border border-white/20 bg-white/10 py-6 text-white shadow-[0_8px_32px_rgba(0,0,0,0.28)] backdrop-blur-xl",
        className
      )}
      {...props}
    />
...
function GlassCardHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "@container/card-header grid auto-rows-min grid-rows-[auto_auto] items-start gap-1.5 px-5 has-data-[slot=glass-card-action]:grid-cols-[1fr_auto] [.border-b]:pb-5",
        className
      )}
      {...props}
    />
...
function GlassCardContent({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div data-slot="glass-card-content" className={cn("px-5", className)} {...props} />
  );
}
