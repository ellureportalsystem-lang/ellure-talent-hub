...
/** Abstract hiring dashboard — no stock photos */
const HeroDashboardMockup = ({ className = "" }: { className?: string }) => (
  <${m}
    ...
    aria-hidden
  >
    <${m} className="hero-mockup-glow absolute -inset-4 rounded-3xl opacity-60 blur-2xl" />
    <${m}
    ...
    >
      ...
      >
        <${d} className="h-2 w-12 rounded bg-white/30 mb-2" />
        <${d} className="h-8 rounded-md bg-secondary/40 mb-1.5" />
        <${d} className="h-2 w-16 rounded bg-white/20" />
      ${cm}
      <${d} className="flex items-center gap-2 mb-3 px-1">
        <${d} className="h-2.5 w-2.5 rounded-full bg-white/40" />
        <${d} className="h-2.5 w-2.5 rounded-full bg-white/25" />
        <${d} className="h-2.5 w-2.5 rounded-full bg-white/25" />
        <${d} className="ml-auto h-2 w-16 sm:w-20 rounded-full bg-white/20" />
      ${cd}
      <${d} className="grid grid-cols-3 gap-1.5 sm:gap-2 mb-3">
        {[1, 2, 3].map((i) => (
          <${m}
            key={i}
          ...
          >
            <${d} className="h-1.5 w-6 sm:w-8 rounded bg-secondary/70 mb-1.5 sm:mb-2" />
            <${d} className="h-3 sm:h-4 w-full rounded bg-white/25" />
          ${cm}
        ))}
      ${cd}
      <${d} className="space-y-1.5 sm:space-y-2">
        {[1, 2, 3, 4].map((i) => (
          <${m}
            key={i}
          ...
          >
            <${d} className="h-7 w-7 sm:h-8 sm:w-8 rounded-full bg-gradient-to-br from-primary/60 to-secondary/60 shrink-0" />
            <${d} className="flex-1 space-y-1 min-w-0">
              <${d} className="h-2 w-3/4 max-w-[100px] sm:max-w-[120px] rounded bg-white/35" />
              <${d} className="h-1.5 w-1/2 max-w-[60px] sm:max-w-[80px] rounded bg-white/20" />
            ${cd}
            <${d} className="h-4 sm:h-5 w-10 sm:w-12 rounded-md bg-secondary/50 shrink-0" />
          ${cm}
        ))}
      ${cd}
    ${cm}
...
// Fix: d should be "motion.div" not "motion.div" - use literal div for static
const fixed = content.replaceAll("<motion.div ", "<div ").replaceAll("<motion.div\n", "<motion.div\n");
// above wrong - rewrite with correct d
const out = content
  .replace(/const d = "motion\.motion\.motion\.div";/, "")
  .replaceAll("<${d}", "<div")
  .replaceAll("${cd}", "</motion.div>")
  .replaceAll("${cm}", "</motion.div>");

// Simpler: just write directly
const final = `import { motion } from "framer-motion";
...
fs.writeFileSync("src/components/marketing/HeroDashboardMockup.tsx", final);
console.log("HeroDashboardMockup written");
